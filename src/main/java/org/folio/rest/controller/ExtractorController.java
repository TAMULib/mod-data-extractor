...
public class ExtractorController implements RepresentationModelProcessor<EntityModel<Extractor>> {
  ...
  @PostMapping("extractors/{id}/run")
  public ResponseEntity<StreamingResponseBody> runExtractor(@PathVariable String id,
      @RequestBody Map<String, String> context, StreamingResponseBody stream) {
  ...
  public EntityModel<Extractor> process(EntityModel<Extractor> resource) {
    resource.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(ExtractorController.class)
        .runExtractor(resource.getContent().getId(), new HashMap<String, String>(), null)).withRel("run"));
    return resource;
  }
